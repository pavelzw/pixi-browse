...
def test_key_focuses_main_panel_from_sidebar(
    compare_snapshot: SnapCompare, make_app: AppFactory, key: str
) -> None:
...
def test_jump_and_page_down_highlight_last_package(
    compare_snapshot: SnapCompare, make_app: AppFactory, keys: tuple[str, ...]
) -> None:
...
def test_extra_depends_tab(
    compare_snapshot: SnapCompare, make_app: AppFactory, package_index: int
) -> None:
    async def run_before(pilot: Pilot[None]) -> None:
