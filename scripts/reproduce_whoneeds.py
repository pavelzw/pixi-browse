...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run consecutive python and polars who-needs queries."
    )
    parser.add_argument(
        "--pause",
        action="store_true",
...
async def main() -> None:
    ...
    print(
        "Platforms: " + ", ".join(str(platform) for platform in CONDA_FORGE_PLATFORMS)
    )
    ...
    for target in TARGETS:
        if current_result:
            print(
                f"\nKeeping {len(current_result):,} results from the previous view "
                f"while querying {target!r}..."
            )
        else:
            print(f"\nQuerying {target!r}...")
        ...
        package_count = len(
            {dependent.record.name.normalized for dependent in current_result}
        )
        print(
            f"Finished {target!r} in {elapsed:.3f}s: "
            f"{len(current_result):,} records across {package_count:,} packages"
        )
